...
export default function RecommendationActionBar({ products, onSendMessage, quickReplies, domain }: RecommendationActionBarProps) {
  ...
  const isLaptops = !domain || domain === 'laptops';
  const noun = isLaptops ? 'laptops' : domain ?? 'products';

  // Build ctx tag from currently shown product IDs so backend focuses on exactly these
  const ctxTag = (() => {
  ...
  return (
    <div className="mt-4 space-y-3">
      ...
      {/* RAG chips — spec-grounded questions generated by the backend */}
      ...
      {/* Common product questions — always shown */}
      <div className="space-y-1.5">
        <p className="text-xs text-black/35 font-medium">Common questions</p>
        <div className="flex flex-wrap gap-1.5">
          {commonQuestions.map((q, idx) => (
            <button
              key={idx}
              onClick={() => onSendMessage(q.msg)}
              className="px-3 py-1.5 text-sm rounded-full border border-black/20 bg-white hover:bg-black/5 text-black/70 hover:text-black font-medium transition-colors"
            >
              {q.label}
            </button>
          ))}
      ...
      {/* Action Buttons Row — de-duplicated */}
